...
function getQueue(body) {
  ...
  queueInstance.getRoutingQueues(opts)
      .then((data) => {
        LoopQueue(data, body)
      })
      .catch((e) => console.error(e))
...
function getFlowMilestone(body) {
  let flowInstance = new platformClient.ArchitectApi();
  flowInstance.getFlowsMilestones(opts)
      .then((data) => {
...
function getFlowOutCome(body){
  let flowInstance = new platformClient.ArchitectApi();
  flowInstance.getFlowsOutcomes(opts)
      .then((data) => {
...
function getDid(body){
  ...
  teleInstance.getTelephonyProvidersEdgesDids(opts)
      .then((data) => {
        LoopDid(data, body)
      })
      .catch((e) => console.error(e))
...
function getWrapUp(body){
  ...
  wrapInstance.getRoutingWrapupcodes(opts)
      .then((data) => {
...
function getSurvey(body){
  ...
  wrapInstance.getQualityFormsSurveys(opts)
      .then((data) => {
  ...
  await process()
      async function process(){
          for await(const component of filter.Dnis){
              var jsonData = fs.readFileSync(__dirname+`/../Payload/Directions/${component}`)
              var payload = JSON.parse(jsonData);          
              Object.assign(payload, { interval: `${yesterday}T00:00:00/${datetime}T00:00:00` })
